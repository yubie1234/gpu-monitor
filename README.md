...
- **노드 필터·검색**: 노드가 많은 클러스터를 위해 이름·장치·워크로드·네임스페이스 검색 +
  유휴 있음 / FULL / NotReady 빠른 토글 (전부 클라이언트 필터, 재요청 없음)
- **데이터 신선도 표시**: 마지막 수집 시각을 상대 시간("N분 전")으로 표시하고, 수집이
  정체되면(스냅샷이 오래됨) 라이브닷을 호박색으로 전환 — HTTP 응답만으로 초록을 유지하지 않는다
- Prometheus `/metrics`
