...
def _node(name, product, cap, alloc=None, shared=None, physical=None,
    ...
    if physical is not None:
        labels["nvidia.com/gpu.count"] = str(physical)
    ...
    for res, qty in (shared or {}).items():
        capacity[res] = str(qty)
        allocatable[res] = str(qty)
    return {
...
def _pod(name, ns, gpu, node="gpu-node-01", labels=None, owner=None, phase="Running",
         ready=True, shared=None):
    """shared: {resource: qty} 공유 슬롯 점유."""
...
class TestGpuPrimitives(unittest.TestCase):
    ...
    def test_node_gpu(self):
        ...
        self.assertEqual(g["shared_pools"], [])
        self.assertIsNone(g["physical"])

    def test_is_gpu_resource_and_profile(self):
    ...
    def test_pod_gpu_resources_whole_and_shared(self):
        ...
        pod = _pod("p", "ns", 2, shared={"nvidia.com/gpu.10gb": 3})
        self.assertEqual(pod_gpu_resources(pod),
                         {"nvidia.com/gpu": 2, "nvidia.com/gpu.10gb": 3})
        self.assertEqual(pod_gpu(pod), 2)  # pod_gpu 는 온전만

    def test_pod_gpu_resources_shared_only(self):
        pod = _pod("p", "ns", 0, shared={"nvidia.com/gpu.10gb": 4})
        self.assertEqual(pod_gpu_resources(pod), {"nvidia.com/gpu.10gb": 4})
        self.assertEqual(pod_gpu(pod), 0)

    def test_node_gpu_shared_pools_and_physical(self):
        # 실환경 재현: 물리 8, 온전 7, .10gb 8슬롯(replicas 8), time-slicing.
        node = _node("h100", "NVIDIA-H100-80GB-HBM3", 7, 7,
                     shared={"nvidia.com/gpu.10gb": 8}, physical=8,
                     replicas=8, sharing="time-slicing")
        g = node_gpu(node)
        self.assertEqual(g["capacity"], 7)
        self.assertEqual(g["physical"], 8)
        self.assertEqual(g["replicas"], 8)
        self.assertEqual(g["sharing_strategy"], "time-slicing")
...
class TestCollect(unittest.TestCase):
    ...
    def test_collect_gpu_nodes_physical_and_backing(self):
        # 물리 8, 온전 7, .10gb 8슬롯 -> shared_backing = 8-7 = 1.
        client = FakeClient({"/api/v1/nodes": {"items": [
            _node("h100", "NVIDIA-H100-80GB-HBM3", 7, 7,
                  shared={"nvidia.com/gpu.10gb": 8}, physical=8,
                  replicas=8, sharing="time-slicing")]}})
        nodes, errs = collect_gpu_nodes(client, {})
        self.assertEqual(errs, [])
        n = nodes[0]
        self.assertEqual(n["capacity"], 7)
        self.assertEqual(n["physical"], 8)
        self.assertEqual(n["shared_backing"], 1)
        self.assertEqual(n["replicas"], 8)
        self.assertEqual(n["sharing_strategy"], "time-slicing")
        self.assertEqual(len(n["shared_pools"]), 1)
        self.assertEqual(n["shared_pools"][0]["capacity"], 8)
        self.assertEqual(n["shared_pools"][0]["allocations"], [])

    def test_collect_gpu_nodes_shared_only_node_kept(self):
        # 온전 nvidia.com/gpu 가 0 이라도 공유 풀/gpu.count 있으면 GPU 노드로 유지.
        client = FakeClient({"/api/v1/nodes": {"items": [
            _node("sliced", "NVIDIA-H100-80GB-HBM3", 0, 0,
                  shared={"nvidia.com/gpu.10gb": 64}, physical=8, replicas=8)]}})
        nodes, errs = collect_gpu_nodes(client, {})
        self.assertEqual([n["name"] for n in nodes], ["sliced"])
        self.assertEqual(nodes[0]["capacity"], 0)
        self.assertEqual(nodes[0]["shared_backing"], 8)  # 8 - 0

    def test_collect_allocations_splits_whole_and_shared(self):
        pods = {"items": [
            _pod("qwen-predictor-0", "kserve", 2,
                 labels={"serving.kserve.io/inferenceservice": "qwen"}),   # 온전 2
            _pod("nb-0", "team-ml", 0, shared={"nvidia.com/gpu.10gb": 2},
                 labels={"notebook-name": "jhwang"}),                      # 슬롯 2
            _pod("batch-x", "team-ml", 0, shared={"nvidia.com/gpu.10gb": 1},
                 owner={"kind": "Job", "name": "batch"}, ready=False),     # 슬롯 1
            _pod("done", "ml", 0, shared={"nvidia.com/gpu.10gb": 4},
                 phase="Succeeded"),                                       # 종료 -> 제외
        ]}
        client = FakeClient({"spec.nodeName=h100": pods})
        node = {"name": "h100", "capacity": 7, "allocatable": 7, "allocated": 0,
                "free": None, "allocations": [], "error": None,
                "shared_pools": [{"resource": "nvidia.com/gpu.10gb", "profile": "10gb",
                                  "capacity": 8, "allocatable": 8, "allocated": 0,
                                  "free": None, "allocations": []}]}
        collect_allocations(client, node)
        # 온전 풀
        self.assertEqual(node["allocated"], 2)
        self.assertEqual(node["free"], 5)
        self.assertEqual(len(node["allocations"]), 1)
        self.assertEqual(node["allocations"][0]["workload_type"], "KServe")
        # 공유 풀
        pool = node["shared_pools"][0]
        self.assertEqual(pool["allocated"], 3)   # 2 + 1 (종료 Pod 제외)
        self.assertEqual(pool["free"], 5)         # 8 - 3
        self.assertEqual(len(pool["allocations"]), 2)
        self.assertEqual(pool["allocations"][0]["slots"], 2)  # 큰 순 정렬

    def test_collect_allocations_creates_missing_pool(self):
        # capacity 엔 없던 공유 리소스를 Pod 이 점유 -> 풀 방어적 생성.
        pods = {"items": [_pod("p", "ns", 0, shared={"nvidia.com/gpu.20gb": 2})]}
        client = FakeClient({"spec.nodeName=h100": pods})
        node = {"name": "h100", "capacity": 7, "allocatable": 7, "allocated": 0,
                "free": None, "allocations": [], "error": None, "shared_pools": []}
        collect_allocations(client, node)
        self.assertEqual(len(node["shared_pools"]), 1)
...
class TestSummarize(unittest.TestCase):
    ...
    def test_products(self):
        s = summarize(self._snap())
        # physical 라벨 없는 노드는 capacity 로 폴백 -> physical == capacity.
        self.assertEqual(s["products"]["H100"],
                         {"capacity": 16, "allocated": 11, "free": 5, "physical": 16})
        self.assertEqual(s["products"]["B200"]["capacity"], 8)

    def test_gpu_physical_falls_back_to_capacity(self):
        # physical 필드 없는 노드 -> gpu_physical == gpu_capacity.
        s = summarize(self._snap())
    ...
    def test_shared_and_physical_aggregation(self):
        snap = {"nodes": [
            {"name": "h100", "product": "H100", "capacity": 7, "allocatable": 7,
             "allocated": 7, "free": 0, "physical": 8, "shared_backing": 1,
             "allocations": [{"workload_type": "KServe", "gpu": 7,
                              "namespace": "kserve", "ready": True}],
             "shared_pools": [{"resource": "nvidia.com/gpu.10gb", "profile": "10gb",
                               "capacity": 8, "allocatable": 8, "allocated": 3,
                               "free": 5, "allocations": []}]},
            {"name": "b200", "product": "B200", "capacity": 8, "allocatable": 8,
             "allocated": 6, "free": 2, "physical": 8, "shared_backing": 0,
             "allocations": [], "shared_pools": []},
        ]}
        s = summarize(snap)
        self.assertEqual(s["gpu_capacity"], 15)     # 온전 7 + 8
        self.assertEqual(s["gpu_physical"], 16)     # 물리 8 + 8
        self.assertEqual(s["gpu_shared_backing"], 1)
        self.assertEqual(s["shared"]["capacity"], 8)
        self.assertEqual(s["shared"]["allocated"], 3)
        self.assertEqual(s["shared"]["free"], 5)
        self.assertEqual(s["shared"]["by_profile"]["10gb"],
...
class TestPrometheus(unittest.TestCase):
    ...
    def test_render_metric_families_grouped(self):
        ...
        self._assert_grouped(render_prometheus_metrics(self._snap()))

    def _assert_grouped(self, text):
        groups, seen = [], set()
    ...
    def _shared_snap(self):
        return {
            "summary": {"gpu_capacity": 15, "gpu_allocated": 13, "gpu_free": 2,
                        "gpu_physical": 16, "gpu_shared_backing": 1, "node_count": 2,
                        "by_workload_type": {"KServe": 13},
                        "by_namespace": {"default": 13},
                        "by_ready": {"true": 13, "false": 0},
                        "shared": {"capacity": 8, "allocated": 3, "free": 5,
                                   "by_profile": {"10gb": {"capacity": 8, "allocated": 3,
                                                           "free": 5}}}},
            "nodes": [
                {"name": "h100", "product": "H100",
                 "product_raw": "NVIDIA-H100-80GB-HBM3", "ready": True,
                ...
                {"name": "b200", "product": "B200", "product_raw": "NVIDIA-B200",
                 "ready": True, "capacity": 8, "allocatable": 8, "allocated": 6,
                 "free": 2, "physical": 8, "shared_backing": 0, "error": None,
                 "shared_pools": []},
            ],
            "k8s_enabled": True, "errors": [],
        }

    def test_render_shared_families(self):
        text = render_prometheus_metrics(self._shared_snap())
        self.assertIn("gpu_monitor_cluster_gpu_physical 16", text)
        self.assertIn('gpu_monitor_cluster_shared_slots{state="capacity"} 8', text)
        self.assertIn('gpu_monitor_cluster_shared_slots{state="allocated"} 3', text)
        self.assertIn('gpu_monitor_cluster_shared_slots{state="free"} 5', text)
        self.assertIn('gpu_monitor_node_physical{node="h100",product="H100"} 8', text)
        self.assertIn('gpu_monitor_node_shared_backing{node="h100",product="H100"} 1',
                      text)
    ...
    def test_render_shared_families_grouped(self):
