# gpu-monitor 컨테이너 이미지 (FastAPI 서비스)
# 수집 로직은 표준 라이브러리(urllib/ssl)만 쓰지만, 웹 계층은 FastAPI 스택을 쓴다.
FROM python:3.12-slim

# 이미지 메타데이터(버전은 빌드 시 --build-arg VERSION 으로 주입; ci.sh 가 채움)
ARG VERSION=dev
LABEL org.opencontainers.image.title="ai-tool/gpu-monitor" \
      org.opencontainers.image.description="노드별 GPU 할당(allocation) 현황 모니터 — Node/GPU/Workload (FastAPI)" \
      org.opencontainers.image.version="${VERSION}" \
      org.opencontainers.image.source="https://github.com/yubie1234/gpu-monitor"
...
# 기본 실행: FastAPI 서비스. 설정은 환경변수(MONITOR_*) / MONITOR_CONFIG_FILE 로 주입.
#
# --timeout-keep-alive 75: nginx ingress 의 upstream keepalive(기본 60s)보다 길게 둔다.
#   uvicorn 기본은 5s 라 폴링 대시보드에선 uvicorn 이 유휴 연결을 닫는 순간 nginx 가
